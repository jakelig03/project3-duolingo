...
void table(const std::vector<node>& array, int numberToDisplay, bool ascending) {
    ...
    while (true) {
        ...
        // display page with rnaking #, learning words, language, and score
...
void compareAlgo (const vector<vector<node>>& array){
    // make copies of given array, without modifying the original
    ...
    for (vector<node> learningLang : array){
        ...
        // Shell Sort
        auto Shellstart = chrono::high_resolution_clock::now();
        shellSort(shellArray);
        auto Shellend = chrono::high_resolution_clock::now();
        Shellduration += Shellend - Shellstart;
